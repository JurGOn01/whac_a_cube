...
void initPollAllButtons(button_t b[]){
  for (int i = 0; i<12;i++){
    initPollButton(&b[i]);
  }
}
void readingAllSensors(button_t b[]){
  for (int i = 0; i<4;i++){
...
void pollAllButtons(button_t b[]){
  for (int i = 0; i<12;i++){
    pollButtonTask(&b[i]);
  }
}
  ...
  for (int i = 0; i<12;i++){
    if(b[i].ev){
      ...
      while(b[i].ev){// need to 'trap' the current button that is pressed
        readingAllSensors(b);
        pollButtonTask(&b[i]);// to update if the user stopped pressing the button.
